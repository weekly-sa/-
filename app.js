...
class AgeTaskTracker {
    ...
    setupEventListeners() {
        ...
        if (document.getElementById('statsBtn')) {
            document.getElementById('statsBtn').addEventListener('click', () => this.showProgressStats());
        }
    ...
    switchView(view) {
        ...
        if (view === 'weeks') {
            this.renderLifeWeeksView();
        } else if (view !== 'day' && view !== 'week' && view !== 'month' && view !== 'year') {
            return;
        } else {
            this.renderCurrentView();
        }
    ...
    renderCurrentView() {
        const views = ['dayView', 'weekView', 'monthView', 'yearView', 'weeksView'];
        views.forEach(view => {
        ...
        const viewMap = { day: 'dayView', week: 'weekView', month: 'monthView', year: 'yearView', weeks: 'weeksView' };
        if (document.getElementById(viewMap[this.currentView])) {
    ...
    renderLifeWeeksView() {
        ...
        const currentWeekNumber = Math.floor((today - birth) / (1000 * 60 * 60 * 24 * 7));
        ...
        const maxWeeks = Math.ceil(80 * 52);

        for (let i = 0; i < maxWeeks; i++) {
            const box = document.createElement('div');
            box.className = 'week-box';
            box.title = `الأسبوع ${i + 1}`;

            if (i < totalWeeks) {
                box.style.backgroundColor = '#2563eb';
                box.style.cursor = 'pointer';
                box.addEventListener('click', () => this.showWeekDetails(i));
    ...
    showWeekDetails(weekIndex) {
        ...
        const isCurrentWeek = Math.floor((today - birth) / (1000 * 60 * 60 * 24 * 7)) === weekIndex;
        const daysInWeek = this.getDaysInWeek(weekStart);

        modal.innerHTML = `
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px;">
                <h2 style="margin: 0; color: #1f2937; font-size: 1.5rem;">الأسبوع ${weekIndex + 1}</h2>
            ...
            <div style="background: #f8fafc; padding: 16px; border-radius: 8px; margin-bottom: 24px;">
                <p style="margin: 0; font-size: 0.875rem; color: #6b7280;">المدة: ${this.formatDateArabic(weekStart)} إلى ${this.formatDateArabic(weekEnd)}</p>
                <p style="margin: 8px 0 0 0; font-size: 0.875rem; color: #6b7280;">الأيام المتبقية: ${daysInWeek}</p>
                ${isCurrentWeek ? '<p style="margin: 8px 0 0 0; color: #10b981; font-weight: 600;">هذا هو أسبوعك الحالي!</p>' : ''}
            </div>

            <div id="weekTasksContainer" style="margin-bottom: 24px;"></div>

            <form id="weekTaskForm" style="border-top: 2px solid #e2e8f0; padding-top: 16px;">
                <div style="margin-bottom: 16px;">
                    <label style="display: block; margin-bottom: 8px; font-weight: 600;">عنوان المهمة</label>
                    <input type="text" id="weekTaskTitle" placeholder="أدخل عنوان المهمة" style="
                        width: 100%;
                        padding: 8px;
                        border: 1px solid #e2e8f0;
                        border-radius: 6px;
                ...
                <div style="margin-bottom: 16px;">
                    <label style="display: block; margin-bottom: 8px; font-weight: 600;">المدة الزمنية (بالدقائق)</label>
                    <input type="number" id="weekTaskDuration" min="1" value="30" style="
                        width: 100%;
                        padding: 8px;
                        border: 1px solid #e2e8f0;
                        border-radius: 6px;
                ...
                <div style="margin-bottom: 16px;">
                    <label style="display: block; margin-bottom: 8px; font-weight: 600;">أضف تذكيرات</label>
                    <div id="remindersContainer" style="display: flex; flex-direction: column; gap: 8px;"></div>
                    <button type="button" onclick="document.getElementById('remindersContainer').innerHTML += '<input type=\"time\" class=\"reminder-time\" style=\"padding: 8px; border: 1px solid #e2e8f0; border-radius: 6px;\">';" style="
                        margin-top: 8px;
                        padding: 8px;
                        background: #f3f4f6;
                        border: 1px solid #e2e8f0;
        ...
        modal.querySelector('#weekTaskForm').addEventListener('submit', (e) => this.handleWeekTaskSubmit(e, weekIndex, weekStart));

        this.displayWeekTasks(weekStart, weekEnd, modal);
    }
    ...
    getDaysInWeek(weekStart) {
        ...
        while (currentDay <= today && currentDay <= weekEnd) {
            daysCount++;
    ...
    displayWeekTasks(weekStart, weekEnd, modal) {
        const container = modal.querySelector('#weekTasksContainer');
        container.innerHTML = '<h3 style="margin-top: 0; color: #1f2937;">المهام في هذا الأسبوع</h3>';

        const weekTasksData = this.tasks.filter(task => {
            const taskStart = new Date(task.start_date);
            return taskStart >= weekStart && taskStart <= weekEnd;
        });

        if (weekTasksData.length === 0) {
            container.innerHTML += '<p style="color: #6b7280; font-size: 0.875rem;">لا توجد مهام في هذا الأسبوع</p>';
        } else {
            ...
            weekTasksData.forEach(task => {
                ...
                taskEl.innerHTML = `
                    <div>
                        ...
                        <div style="font-size: 0.75rem; color: #6b7280; margin-top: 4px;">${task.duration_minutes || 0} دقيقة</div>
                    </div>
                    <button class="delete-week-task" data-task-id="${task.id}" style="
                        background: #ef4444;
                        color: white;
                        border: none;
                        padding: 6px 12px;
                ...
                taskEl.querySelector('.delete-week-task').addEventListener('click', (e) => {
    ...
    async handleWeekTaskSubmit(e, weekIndex, weekStart, modal) {
        e.preventDefault();

        const title = document.getElementById('weekTaskTitle').value;
        const duration = parseInt(document.getElementById('weekTaskDuration').value) || 30;
        const reminderInputs = document.querySelectorAll('.reminder-time');
        const reminders = Array.from(reminderInputs).map(input => ({
            time: input.value || '08:00',
            type: 'notification'
        }));
        ...
        try {
            ...
            const { error } = await supabase.from('advanced_tasks').insert([{
                user_id: this.currentUser.id,
                title: title.trim(),
                frequency: 'weekly',
            ...
            await this.loadUserData();
            this.showWeekDetails(weekIndex);
    ...
    calculateProgressStats() {
        ...
        this.tasks.forEach(task => {
            ...
            if (this.isToday(taskDate)) {
                stats.daily.total++;
                if (this.completions.some(c => c.task_id === task.id &&
                    c.completion_date === today.toISOString().split('T')[0])) {
                    stats.daily.completed++;
                }
            }

            if (this.isSameWeek(taskDate, today)) {
                stats.weekly.total++;
                if (this.completions.some(c => c.task_id === task.id)) {
    ...
    showProgressStats() {
        ...
        const updateStats = (period, data) => {
            const percentage = data.total > 0 ? Math.round((data.completed / data.total) * 100) : 0;
            const periodMap = {
                daily: { elem: 'dailyProgress', statsElem: 'dailyStats' },
                weekly: { elem: 'weeklyProgress', statsElem: 'weeklyStats' },
                monthly: { elem: 'monthlyProgress', statsElem: 'monthlyStats' },
                yearly: { elem: 'yearlyProgress', statsElem: 'yearlyStats' }
            };

            const mapping = periodMap[period];
            if (mapping) {
                const elem = document.getElementById(mapping.elem);
                const statsElem = document.getElementById(mapping.statsElem);

                if (elem) elem.textContent = `${percentage}%`;
                if (statsElem) {
                    statsElem.textContent = `${data.completed} من ${data.total} مهام مكتملة`;
                }
            }
        };

        updateStats('daily', stats.daily);
        updateStats('weekly', stats.weekly);
        updateStats('monthly', stats.monthly);
        updateStats('yearly', stats.yearly);

        statsDiv.style.display = 'block';

        window.scrollTo({ top: statsDiv.offsetTop, behavior: 'smooth' });
    }
}
